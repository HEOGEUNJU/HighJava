...
public class MyBatisTest {
	...
	public static void main(String[] args) {
		...
		// 2-1. insert 연습
		System.out.println("insert 작업 연습...");
...
//		try {
//			
//			int cnt = sqlSession.insert("memberTest.insertMember", mv); //memberTest <= namespace / insertMember <= 쿼리아이디
//			
//			if(cnt > 0) {
...
//		System.out.println("------------------------------------------");
		...
		// 2-2 update 연습
		System.out.println("update 작업 연습...");
		...
		try {
			...
			throw new RuntimeException("수정 중 예외 발생", e);
		} finally {
			sqlSession.close(); // 세션 닫음
		}
		...
		// 응답 결과가 여러개인 경우에는 selectList 메서드를 사용한다.
		try {
			memList = sqlSession.selectList("memberTest.selectAllMember");

			if (memList.size() == 0) {
				System.out.println("조회된 저옵가 없습니다.");
			} else {
		...
		MemberVO mv4 = (MemberVO)sqlSession.selectOne("memberTest.selectMember", "heo");
