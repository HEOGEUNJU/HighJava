...
public class T02ServletTest extends HttpServlet{
	 ...
	 *     객체의 service()메서드를 호출하도록 한다.
	 *     (이때, HttpServletRequest 및 HttpServletResponse객체를 생성하여 파라미터로
	 *     넘겨준다.)
	 *  4. service()메서드는 메서드타입을 체크하여 적절한 메서드를 호출한다.
	 *     (doGet, doPost, doPut, doDelete 등)
	 *  5. 요청처리가 완료되면 요청 및 응답 객체는 소멸한다.
	 *  6. 컨테이너로부터 서블릿 객체가 제거된는 경우에는 destroy()가 호출됨.
	 */
	...
	protected void doGet(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException {
		...
		// 반드시 Request 객체에서 값을 가져오기 전에 먼저 설정해야함.
		req.setCharacterEncoding("UTF-8");
		
		// 요청 메세지로부터 name 속성값 가져옴.
		...
		// 저장한 데이터를 꺼내는 법(getAttribute(데이터명))
		...
		//writer, reader는 문자기반 stream 
		//outputstream, inputstream은 바이트기반 stream
